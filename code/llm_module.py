...
try:
    ...
    genai = None
    logging.warning("🤖⚠️ google-genai library not installed. Gemini backend will not function.")
...
class LLM:
    ...
    """
    SUPPORTED_BACKENDS = ["ollama", "openai", "lmstudio", "gemini", "groq"]
    ...
    def __init__(
        ...
        if self.backend == "gemini" and not GENAI_AVAILABLE:
            raise ImportError("google-genai library is required for 'gemini' backend but not installed.")
        if self.backend == "groq" and not GROQ_AVAILABLE:
            raise ImportError("groq library is required for 'groq' backend but not installed.")
    ...
    def _lazy_initialize_clients(self) -> bool:
        ...
        if self._client_initialized:
            ...
            if self.backend == "gemini": return self.gemini_client is not None
            if self.backend == "groq": return self.groq_client is not None
    ...
    def prewarm(self, max_retries: int = 1) -> bool:
        ...
        with Progress(TextColumn("{task.description}"), BarColumn()) as progress:
            task = progress.add_task("[dim]▸ Testing artificial substrate integrity[/dim]", total=None)
            ...
            while attempts <= max_retries:
                ...
                try:
                    ...
                    if attempts < max_retries:
                        ...
                        progress.update(task, description=f"[yellow]Retrying in {wait_time}s...")
                        return False
                    ...
                    progress.update(task, description="[red]API error")
                    return False 
                except Exception as e:
                    last_error = e
            ...
            progress.update(task, description="[red]LLM isnt available")
            return False
        ...
        **kwargs: Any
        ...
        try:
            if self.backend == "openai":
                ...
                if use_system_prompt and self.system_prompt_message and gemini_contents:
                    first_user_content = gemini_contents[0]["parts"][0]["text"]
                    gemini_contents[0]["parts"][0]["text"] = f"{self.system_prompt}\n\n{first_user_content}"
                ...
                yield from self._yield_gemini_chunks(stream_iterator, req_id)
                ...
                stream_iterator = self.groq_client.chat.completions.create(
                    model=self.model,
                    messages=messages,
                    stream=True,
                    **kwargs
        ...
        # Catch specific exceptions first
            ...
            console.log(f"API/Request Error during generation")
            ...
            console.log(f"🤖💥 Unexpected error in generation pipeline for {req_id}: {e}", exc_info=True) # Log traceback for unexpected
    ...
    def _yield_gemini_chunks(self, stream, request_id: str) -> Generator[str, None, None]:
        """Yields content chunks from Gemini stream."""
        token_count = 0
        try:
            for chunk in stream:
                with self._requests_lock:
                    if request_id not in self._active_requests:
                        logger.info(f"🤖🗑️ Gemini stream {request_id} cancelled.")
                        break
            ...
            logger.debug(f"🤖✅ [{request_id}] Finished yielding {token_count} Gemini tokens.")
        except Exception as e:
            is_cancelled = False
            with self._requests_lock:
                is_cancelled = request_id not in self._active_requests
            if is_cancelled:
                logger.warning(f"🤖⚠️ Gemini stream error likely due to cancellation for {request_id}: {e}")
            else:
                logger.error(f"🤖💥 Error during Gemini streaming ({request_id}): {e}", exc_info=True)
                raise
        finally:
            if stream and hasattr(stream, 'close') and callable(stream.close):
                try:
                    logger.debug(f"🤖🗑️ [{request_id}] Closing Gemini stream.")
                    stream.close()
                except Exception as close_err:
                    logger.warning(f"🤖⚠️ [{request_id}] Error closing Gemini stream: {close_err}", exc_info=False)
    ...
    def measure_inference_time(
        ...
        **kwargs: Any
        ...
        console = Console()
        with Progress(TextColumn("{task.description}"), BarColumn()) as progress:
            task = progress.add_task("[dim]▸ Measuring LLM latency[/dim]", total=None)
            ...
            try:
                ...
                console.log(f"🤖⏱️💥 Error during inference time measurement ({req_id}): {e}", exc_info=False)
            ...
            if duration_ms > 400:
                progress.update(task, description=f"[red]LLM inference speed is WAY TOO SLOW")
            else:
                progress.update(task, description=f"[cyan]LLM inference speed is fast enough")
