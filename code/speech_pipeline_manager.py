...
class SpeechPipelineManager:
    ...
    def __init__(
        ...
        # Load system prompt
        try:
            with open(f"resources/system_prompt_{language}.txt", "r", encoding="utf-8") as f:
                self.system_prompt = f.read().strip()
        except FileNotFoundError:
            console.log(f"[yellow]⚠️  resources/system_prompt_{language}.txt not found. Using default.")
        ...
        try:
            ...
            console.log(f"[red]💥 Failed to initialize AudioProcessor: {e}")
            raise
        ...
        try:
            ...
            console.log(f"[red]💥 Failed to initialize LLM: {e}")
            raise
        ...
        try:
            ...
            console.log(f"[green]✅ LLM ready (inference: {self.llm_inference_time:.0f}ms)")
        except Exception as e:
            console.log(f"[red]💥 Failed during LLM prewarm/measurement: {e}")
            raise
        ...
        # Calculate full pipeline latency
            ...
            try:
                ...
                if self.previous_request:
                    ...
                    if self.previous_request.data == request.data and isinstance(request.data, str):
                        if request.timestamp - self.previous_request.timestamp < 2:
                            logger.info(f"ðŸ—£ï¸ðŸ—‘ï¸ Request Processor: Skipping duplicate request - {request.action}")
                            continue

                # Drain the queue to get the most recent request
                while not self.requests_queue.empty():
                    skipped_request = self.requests_queue.get(False)  # Non-blocking get
                    logger.debug(f"ðŸ—£ï¸ðŸ—‘ï¸ Request Processor: Skipping older request - {skipped_request.action}")
                    request = skipped_request # Keep the last one we retrieved
                
                self.abort_block_event.wait() # Wait if an abort is in progress
                logger.debug(f"ðŸ—£ï¸ðŸ”„ Request Processor: Processing most recent request - {request.action}")
                ...
                if request.action == "prepare":
                    ...
                    self.previous_request = request
                     ...
                     logger.info(f"ðŸ—£ï¸ðŸ¤· Request Processor: Received 'finish' action (currently no-op).")
                     self.previous_request = request # Still update previous_request
                else:
                    logger.warning(f"ðŸ—£ï¸â“ Request Processor: Unknown action '{request.action}'")

            except Empty:
                continue
            except Exception as e:
                logger.exception(f"ðŸ—£ï¸ðŸ’¥ Request Processor: Error: {e}")
        logger.info("ðŸ—£ï¸ðŸ Request Processor: Shutting down.")
    ...
    def on_first_audio_chunk_synthesize(self):
        ...
        if self.running_generation:
            ...
            self.latency_tracker.mark_tts_first_chunk()
    ...
    def preprocess_chunk(self, chunk: str) -> str:
        ...
        """
        return chunk.replace("—", "-").replace("“", '"').replace("”", '"').replace("‘", "'").replace("’", "'").replace("…", "...").replace("*", "")

    def clean_quick_answer(self, text: str) -> str:
        """
        Removes specific leading patterns (like newlines, spaces) from text.
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                for chunk in current_gen.llm_generator:
                    ...
                    # console.log(f"💄🔍 CHUNK BEFORE: '{chunk}'")
                    chunk = self.preprocess_chunk(chunk) # Normalization chunk by chunk
                    # console.log(f"💄🔍 After CHUNK: '{chunk}'")
                    token_count += 1
                    current_gen.quick_answer += chunk

                    if token_count == 1:
                        self.latency_tracker.mark_llm_first_token() 
                        # console.log(f"🔍 LLM FIRST TOKEN in {(time.time()-gen_start)*1000:.0f}ms")

                    # Check for quick answer boundary only if not already provided
                    if not current_gen.quick_answer_provided:
                        raw_text = current_gen.quick_answer
                        
                        # Skip if text ends with incomplete time (digit(s) followed by colon)
                        # This prevents splitting "20:" before "00" arrives
                        if re.search(r'\d{1,2}:$', raw_text):
                        ...
                        if context:
                            self.latency_tracker.mark_llm_context_ready(len(context))  
                ...
                logger.info(f"🔍 DEBUG: After LLM loop - llm_generator is: {current_gen.llm_generator}")
                logger.info(f"🔍 DEBUG: llm_aborted: {current_gen.llm_aborted}")
                logger.info(f"🔍 DEBUG: quick_answer_provided: {current_gen.quick_answer_provided}")

                # Loop finished naturally or broke due to stop request
                logger.info(f"ðŸ—£ï¸ðŸ§ ðŸ [Gen {gen_id}] LLM Worker: Generator loop finished%s" % (" (Aborted)" if current_gen.llm_aborted else ""))
                ...
                if not current_gen.llm_aborted and not current_gen.quick_answer_provided:
                    logger.info(f"ðŸ—£ï¸ðŸ§ âœ”ï¸ [Gen {gen_id}] LLM Worker: No context boundary found, using full response as quick answer.")
                ...
                logger.exception(f"ðŸ—£ï¸ðŸ§ ðŸ’¥ [Gen {gen_id}] LLM Worker: Error during generation: {e}")
                ...
                if current_gen.llm_aborted:
                    ...
                    logger.info(f"ðŸ—£ï¸ðŸ§ âŒ [Gen {gen_id}] LLM Aborted, requesting TTS quick/final stop.")
                ...
                elif not current_gen.llm_aborted:
                    full_answer = current_gen.quick_answer + current_gen.quick_answer_overhang
                    self.latency_tracker.mark_llm_complete(len(full_answer))  
                logger.info(f"ðŸ—£ï¸ðŸ§ ðŸ [Gen {gen_id}] LLM Worker: Finished processing cycle.")
    ...
    def check_abort(self, txt: str, wait_for_finish: bool = True, abort_reason: str = "unknown") -> bool:
        ...
        with self.check_abort_lock:
            if self.running_generation:
                ...
                logger.info(f"ðŸ—£ï¸ðŸ›‘â“ {current_gen_id_str} Abort check requested (reason: {abort_reason})")

                if self.running_generation.abortion_started:
                    logger.info(f"ðŸ—£ï¸ðŸ›‘â³ {current_gen_id_str} Active generation is already aborting, waiting to finish (if requested).")
                    ...
                    if wait_for_finish:
                        ...
                        if not completed:
                             logger.error(f"ðŸ—£ï¸ðŸ›‘ðŸ’¥ðŸ’¥ {current_gen_id_str} Timeout waiting for ongoing abortion to complete. State inconsistency possible!")
                            ...
                            logger.error(f"ðŸ—£ï¸ðŸ›‘ðŸ’¥ðŸ’¥ {current_gen_id_str} Abortion completed event set, but running_generation still exists. State inconsistency likely!")
                            # Force clear it.
                            self.running_generation = None
                        else:
                            logger.info(f"ðŸ—£ï¸ðŸ›‘âœ… {current_gen_id_str} Ongoing abortion finished.")
                    else:
                        logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸƒ {current_gen_id_str} Not waiting for ongoing abortion as wait_for_finish=False")

                    return True # An abort was processed (waited for)
                else:
                    # No abortion in progress, check similarity
                    logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ¤” {current_gen_id_str} Found active generation, checking text similarity.")
                    try:
                         # Ensure running_generation.text is not None before comparison
                        if self.running_generation.text is None:
                            logger.warning(f"ðŸ—£ï¸ðŸ›‘â“ {current_gen_id_str} Running generation text is None, cannot compare similarity. Assuming different.")
                            similarity = 0.0
                        else:
                            similarity = self.text_similarity.calculate_similarity(self.running_generation.text, txt)
                    except Exception as e:
                        logger.warning(f"ðŸ—£ï¸ðŸ›‘ðŸ’¥ {current_gen_id_str} Error calculating similarity: {e}. Assuming different.")
                        similarity = 0.0 # Assume different on error

                    if similarity >= 0.95:
                        logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ™… {current_gen_id_str} Text ('{txt[:30]}...') too similar ({similarity:.2f}) to current '{self.running_generation.text[:30] if self.running_generation.text else 'None'}...'. Ignoring.")
                        return False # No abort needed

                    # Texts are different enough, initiate abort
                    logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸš€ {current_gen_id_str} Text ('{txt[:30]}...') different enough ({similarity:.2f}) from '{self.running_generation.text[:30] if self.running_generation.text else 'None'}...'. Requesting synchronous abort.")
                    start_time = time.time()
                    ...
                    if wait_for_finish:
                        ...
                        if self.running_generation is not None:
                            logger.error(f"ðŸ—£ï¸ðŸ›‘ðŸ’¥ðŸ’¥ {current_gen_id_str} !!! Abort call completed but running_generation is still not None. State inconsistency likely!")
                            # Force clear it.
                            self.running_generation = None
                        else:
                            logger.info(f"ðŸ—£ï¸ðŸ›‘âœ… {current_gen_id_str} Synchronous abort completed in {time.time() - start_time:.2f}s.")

                    return True # An abort was processed (initiated)
            else:
                logger.info("ðŸ—£ï¸ðŸ›‘ðŸ¤· No active generation found during abort check.")
    ...
    def process_prepare_generation(self, txt: str):
        ...
        """
        logger.info(f"ðŸ—£ï¸ðŸ”¥ PREPARE_GENERATION CALLED with text: '{txt}'")
        ...
        time_since_ai = time.time() - self.last_ai_speech_end_time
        ...
        self.latency_tracker.start_new_turn(new_gen_id)
        ...
        with self.abort_lock:
            ...
            if current_gen_obj is None or current_gen_obj.abortion_started:
                if current_gen_obj is None:
                    logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ¤· {current_gen_id_str} No active generation found to abort.")
                else:
                    logger.info(f"ðŸ—£ï¸ðŸ›‘â³ {current_gen_id_str} Abortion already in progress.")
            ...
            # --- Start Abort Process ---
            logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸš€ {current_gen_id_str} Abortion process starting...")
            ...
            if is_llm_potentially_active:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ§ âŒ {current_gen_id_str} - Stopping LLM...")
                ...
                if stopped:
                    logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ§ ðŸ‘ {current_gen_id_str} LLM stopped confirmation received.")
                    self.stop_llm_finished_event.clear() # Reset for next time
                else:
                    logger.warning(f"ðŸ—£ï¸ðŸ›‘ðŸ§ â±ï¸ {current_gen_id_str} Timeout waiting for LLM stop confirmation.")
                # Attempt external cancellation if available
                if hasattr(self.llm, 'cancel_generation'):
                    logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ§ ðŸ”Œ {current_gen_id_str} Calling external LLM cancel_generation.")
                    try:
                        self.llm.cancel_generation()
                    except Exception as cancel_e:
                         logger.warning(f"ðŸ—£ï¸ðŸ›‘ðŸ§ ðŸ’¥ {current_gen_id_str} Error during external LLM cancel: {cancel_e}")
                self.llm_generation_active = False # Ensure flag is off
                aborted_something = True
            else:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ§ ðŸ“´ {current_gen_id_str} LLM appears inactive, no stop needed.")
            ...
            if is_tts_quick_potentially_active:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ‘„âŒ {current_gen_id_str} Stopping Quick TTS...")
                self.stop_tts_quick_request_event.set()
                ...
                if stopped:
                    logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ‘„ðŸ‘ {current_gen_id_str} Quick TTS stopped confirmation received.")
                    self.stop_tts_quick_finished_event.clear() # Reset
                else:
                    logger.warning(f"ðŸ—£ï¸ðŸ›‘ðŸ‘„â±ï¸ {current_gen_id_str} Timeout waiting for Quick TTS stop confirmation.")
                self.tts_quick_generation_active = False # Ensure flag is off
                aborted_something = True
            else:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ‘„ðŸ“´ {current_gen_id_str} Quick TTS appears inactive, no stop needed.")
            ...
            if is_tts_final_potentially_active:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ‘„âŒ {current_gen_id_str} Stopping Final TTS...")
                self.stop_tts_final_request_event.set()
                ...
                if stopped:
                    logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ‘„ðŸ‘ {current_gen_id_str} Final TTS stopped confirmation received.")
                    self.stop_tts_final_finished_event.clear() # Reset
                else:
                    logger.warning(f"ðŸ—£ï¸ðŸ›‘ðŸ‘„â±ï¸ {current_gen_id_str} Timeout waiting for Final TTS stop confirmation.")
                self.tts_final_generation_active = False # Ensure flag is off
                aborted_something = True
            else:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ‘„ðŸ“´ {current_gen_id_str} Final TTS appears inactive, no stop needed.")
            ...
            if hasattr(self.audio, 'stop_playback'):
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ”Š {current_gen_id_str} Requesting audio playback stop.")
                try:
                    self.audio.stop_playback() # Or similar method
                except Exception as audio_e:
                    logger.warning(f"ðŸ—£ï¸ðŸ›‘ðŸ”ŠðŸ’¥ {current_gen_id_str} Error stopping audio playback: {audio_e}")
            ...
            if self.running_generation is not None and self.running_generation.id == current_gen_obj.id:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ§¹ {current_gen_id_str} Clearing running generation object.")
                ...
                self.running_generation = None # Clear the reference
                 ...
                 logger.warning(f"ðŸ—£ï¸ðŸ›‘â“ {current_gen_id_str} Mismatch: self.running_generation changed during abort (now Gen {self.running_generation.id}). Clearing current ref.")
                 self.running_generation = None # Clear the unexpected new one too? Or just log? Clearing seems safer.
            elif aborted_something:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ¤· {current_gen_id_str} Worker(s) aborted but running_generation was already None.")
            else:
                logger.info(f"ðŸ—£ï¸ðŸ›‘ðŸ¤· {current_gen_id_str} Nothing seemed active to abort, running_generation is None.")
            ...
            # --- Signal Completion ---
            logger.info(f"ðŸ—£ï¸ðŸ›‘âœ… {current_gen_id_str} Abort processing complete. Setting completion event and releasing block.")
    ...
    def prepare_generation(self, txt: str):
        ...
        """
        start = time.time()
        logger.info(f"🔍 PREPARE START: '{txt[:30]}'")
        logger.info(f"ðŸ—£ï¸ðŸ“¥ Queueing 'prepare' request for: '{txt[:50]}...'")
        self.requests_queue.put(PipelineRequest("prepare", txt))
        logger.info(f"🔍 PREPARE DONE in {(time.time()-start)*1000:.0f}ms")
    ...
    def finish_generation(self):
        ...
        """
        logger.info(f"ðŸ—£ï¸ðŸ“¥ Queueing 'finish' request")
    ...
    def reset(self):
        ...
        """
        logger.info("ðŸ—£ï¸ðŸ”„ Resetting pipeline state...")
        self.abort_generation(wait_for_completion=True, timeout=7.0, reason="reset") # Ensure clean slate
        self.history = []
        logger.info("ðŸ—£ï¸ðŸ§¹ History cleared. Reset complete.")
