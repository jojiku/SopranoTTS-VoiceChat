...
DEFAULT_RECORDER_CONFIG: Dict[str, Any] = {
    ...
    "spinner": False,
    "model": model_whisper,
    "realtime_model_type": model_whisper,
    "use_main_model_for_realtime": False,
    "language": language, # Default, will be overridden by source_language in __init__
    ...
    "debug_mode": True,
    "initial_prompt_realtime": "Hello how are you",
            ...
            while not self.shutdown_performed:
                ...
                if self.recorder and speech_end_silence_start is not None and speech_end_silence_start != 0:
                    ...
                    if now - last_log_time >= LOG_INTERVAL:
                        last_log_time = now
                        logger.info(f"👂⏱️  SILENCE TIMING:")
                        logger.info(f"  Time since silence: {time_since_silence:.3f}s")
                        logger.info(f"  Silence waiting time: {silence_waiting_time:.3f}s")
                        logger.info(f"  Hot threshold: {start_hot_condition_time:.3f}s")
                        logger.info(f"  Hot state active: {hot}")
