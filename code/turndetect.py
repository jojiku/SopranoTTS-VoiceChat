...
class TurnDetection:
    ...
    def get_completion_probability(
        ...
        # Return 0.5 so the model component contributes nothing
        # All detection comes from punctuation
            return 0.5
    ...
    def _text_worker(
        ...
        while True:
            ...
            is_cyrillic = any('\u0400' <= c <= '\u04FF' for c in processed_text)
            ...
            if is_cyrillic:
                weight_towards_whisper = 1.0  # 100% punctuation-based for Russian
            else:
                weight_towards_whisper = 0.65 # Original English weighting
            ...
            logger.info(f"🎤📊 Calculated pauses: Punct={whisper_suggested_pause:.2f}, Model={sentence_finished_model_pause:.2f}, Weighted={weighted_pause:.2f}, Final={final_pause:.2f} for \"{processed_text}\" (Prob={prob_complete:.2f}), Pipeline minimum: {min_pause:.3f}s")
